...
export function ProtectedRoute({ children }: { children: React.ReactNode }) {
  ...
  const isCreateWorkspace = location.pathname === "/create-workspace";
  const isInvite = location.pathname.startsWith("/invite/");

  if (!hasMemberships && !isOnboarding && !isCreateWorkspace && !isInvite) {
    return <Navigate to="/onboarding" replace />;
  }

  if (hasMemberships && (isOnboarding || isCreateWorkspace)) {
