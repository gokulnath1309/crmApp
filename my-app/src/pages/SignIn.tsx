...
import { BarChart2, Eye, EyeOff, CheckCircle, ArrowRight, Star } from "lucide-react";
...
export function SignInPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row bg-slate-50 dark:bg-slate-950">
      <AuthLeft
        headline={<>Manage Leads, Deals &amp; Customer Relationships <span className="text-indigo-300">in One Place</span></>}
        sub="Streamline your pipeline, track every interaction, and close more deals with our AI-powered CRM platform."
      />
      <div className="flex-1 flex items-center justify-center p-6 lg:p-12">
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }} className="w-full max-w-[400px]">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl shadow-slate-200/60 dark:shadow-black/40 p-8 border border-slate-100 dark:border-slate-700/60">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm mb-7">Sign in to your CRM Pro account</p>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <div className="flex justify-between mb-1.5">
                  ...
                  <button type="button" className="text-xs text-indigo-600 dark:text-indigo-400 hover:underline font-medium">Forgot password?</button>
                </div>
                <div className="relative">
                  <input type={showPassword ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)} placeholder="Enter your password" required className={`${field} pr-10`} />
              ...
              <label className="flex items-center gap-2 cursor-pointer select-none">
                <input type="checkbox" checked={remember} onChange={e => setRemember(e.target.checked)} className="w-4 h-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" />
              ...
              <button disabled={loading} type="submit" className="w-full flex items-center justify-center gap-2 py-2.5 bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-400 text-white font-semibold rounded-xl transition-all text-sm shadow-md shadow-indigo-200 dark:shadow-indigo-900/30">
            ...
            <p className="text-center text-sm text-slate-500 dark:text-slate-400 mt-5">
              ...
              <Link to="/signup" className="text-indigo-600 dark:text-indigo-400 font-semibold hover:underline">Create Account</Link>
            </p>
          </div>
        </motion.div>
      </div>
